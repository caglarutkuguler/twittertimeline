...
$(document).ready(function () {
	...
	loadingHTML += "<div id="loading-container"><img src="{$module_dir}views/img/ajax-loader.gif" width="32" height="32" alt="tweet loader" /></div>";
        ...
        function(feeds) {   
		   //alert(feeds);
			...
			function animatetweets() {
				{/literal}
				var tweetdelaytime = {$transitionspeed};
				var tweetfadetime = {$fadingspeed};
				var fadeoffsetin = 15;
				var fadeoffsetout = -15;				
				
				var starttweet = {$startfrom};
...
{
    $( "#wrap_twitter_widget" ).insertBefore( "#footer" ); 
}
</script>


<div id="wrap_twitter_widget" >

<div id="twitter_bird" align="center">
<div id="twitter_bird_img" style="background-image:url({$module_dir}views/img/twitter_birds_empty.png); background-size: 61px 49px;background-color:{$birdcolor};width:61px;height:49px;"></div>
</div>

<div id="twitter-feed"></div>

<div id="twitter_logo" align="center">
<div id="twitter_logo_img"><img src="{$module_dir}views/img/twitter.png" width="120" height="28" alt="Twitter Logo"></div>
</div>
...
//	footerheight = $('.footer-container').height();
	widgetheight = $('#twitter-feed').height();
  	wrapHeight = widgetheight + 55;
//	$('.footer-container').css("height", footerheight+widgetheight);	 
//	
    $( "#wrap_twitter_widget" ).prependTo( "#footer" );
...
function onresize (){
	widgetheight = $('#twitter-feed').height();
  	wrapHeight = widgetheight + 55;
	$('#wrap_twitter_widget, #twitter_bird, #twitter_logo').height(wrapHeight);
	$('#twitter_bird_img, #twitter_logo_img').css("margin-top", wrapHeight/4);
	a =$( window ).width();
	if(a<610) {
		$('#twitter_bird, #twitter_logo').css("display", "none");		
		$('#twitter-feed').css("width", "100%"); 
	} 
	else {
		$('#twitter_bird, #twitter_logo').css("display", "block");			
		$('#twitter-feed').css("width", "60%"); 
	}
}
